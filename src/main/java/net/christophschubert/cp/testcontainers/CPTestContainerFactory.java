...
public class CPTestContainerFactory {
  ...
  public LdapContainer createLdap(Set<String> userNames) {
    try (LdapContainer ldapContainer = new LdapContainer(userNames).withNetwork(network)) {
      return ldapContainer;
    }
  }

  public LdapContainer createLdap() {
    try (LdapContainer ldapContainer = new LdapContainer().withNetwork(network)) {
      return ldapContainer;
    }
  }

  public KafkaContainer createKafka() {
    try (KafkaContainer kafkaContainer = new KafkaContainer(imageName("cp-kafka")).withNetwork(network)) {
      return kafkaContainer;
    }
  }

  public ConfluentServerContainer createConfluentServer() {
    try (ConfluentServerContainer container = (ConfluentServerContainer) new ConfluentServerContainer(repository, tag).withNetwork(network)) {
      return container;
    }
  }

  public SchemaRegistryContainer createSchemaRegistry(KafkaContainer bootstrap) {
    try (var schemaRegistryContainer = new SchemaRegistryContainer(imageName("cp-schema-registry"), bootstrap, network)) {
      return schemaRegistryContainer;
    }
  }

  public KafkaConnectContainer createKafkaConnect(KafkaContainer bootstrap) {
    try (var kafkaConnectContainer = new KafkaConnectContainer(imageName("cp-kafka-connect"), bootstrap, network)) {
  ...
  public ConfluentServerConnectContainer createConfluentServerConnect(ConfluentServerContainer bootstrap) {
    try (var container = new ConfluentServerConnectContainer(imageName("cp-server-connect"), bootstrap, network)) {
      return container;
    }
  }
  ...
  public KafkaConnectContainer createReplicator(KafkaContainer bootstrap) {
    try (var kafkaConnectContainer = new KafkaConnectContainer(imageName("cp-enterprise-replicator"), bootstrap, network)) {
      return kafkaConnectContainer;
    }
  }

  public KafkaConnectContainer createCustomConnector(String hubComponent, KafkaContainer bootstrap) {
    try (var connector = createCustomConnector(Collections.singleton(hubComponent), bootstrap)) {
      return connector;
    }
  }
  ...
  public KafkaConnectContainer createCustomConnector(Set<String> hubComponents, KafkaContainer bootstrap) {
    ...
    try (var connectContainer = new KafkaConnectContainer(image, bootstrap, network).withEnv("CONNECT_PLUGIN_PATH", "/usr/share/confluent-hub-components")) {
      return connectContainer;
    }
  }

  public RestProxyContainer createRestProxy(KafkaContainer bootstrap) {
    try (var restProxyContainer = new RestProxyContainer(imageName("cp-kafka-rest"), bootstrap, network)) {
      return restProxyContainer;
    }
  }
  ...
  public KsqlDBContainer createKsqlDB(KafkaContainer bootstrap) {
    try (var ksqlDBContainer = new KsqlDBContainer(imageName("cp-ksqldb-server"), bootstrap, network)) {
      return ksqlDBContainer;
    }
  }
  ...
  public KsqlDBContainer createKsqDB(KafkaContainer bootstrap, String tag) {
    ...
    try (KsqlDBContainer ksqlDBContainer = new KsqlDBContainer(imageName, bootstrap, network)) {
      return ksqlDBContainer;
    }
  }
  ...
  public ClusterSpec<KafkaContainer> createKafkaCluster(int numBrokers) {
    ...
    try (ZooKeeperContainer zk = new ZooKeeperContainer(repository, tag).withNetwork(network)) {
      ...
      final var kafkas = IntStream.rangeClosed(1, numBrokers)
          .mapToObj(id -> createKafka().
              withNetwork(network).
              dependsOn(zk).
